...
// Q-9- leetcode-122 => You are given an integer array prices where prices[i] is the price of a given stock on the ith day.
...
function getMaxProfitSum(arr) {
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        let checkProfiVal = arr[i] - buy;
        if (checkProfiVal > profit) {
            profit = checkProfiVal;
            totalProfit += profit;
            buy = arr[i];
            profit = 0;
        }
    ...
    console.log("total profit to buy and sell stock=>", totalProfit);
}

getMaxProfitSum([7, 1, 5, 3, 6, 4]);
getMaxProfitSum([1, 2, 3, 4, 5]);
getMaxProfitSum([7, 6, 4, 3, 1]);